...
Based on this figure, it seems the Month and the years have the largest impact on the median ride duration. The median ride duration in September is `{r summarize(median(duration, month="January"))}` and in July it is `{r summarize(median(duration, month="July"))}` The median ride duration was higher in 2020 than in 2024.The median ride duration in 2020 was `{r summarize(median(duration, year="2020"))}` and in 2024 it was {r summarize(median(duration, year ="2024"))}`. The yearly difference might be explained by the COVID-19 pandemic where people preferred bikes over public transportation. The differences between the two months may be explained by weather and users preferring to ride bikes in the summer instead of the winter. 
...
ggplot(aes(x = month, y = duration, fill = rideable_type)) + 
  geom_boxplot() +
   labs(
      title = "Duration Distribution by Month",
      x = "Month",
